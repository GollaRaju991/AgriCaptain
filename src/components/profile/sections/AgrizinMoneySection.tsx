...
const AgrizinMoneySection: React.FC = () => {
  const { user } = useAuth();
  ...
  const handleRecharge = async () => {
    ...
    setProcessing(true);
    try {
      // Open Razorpay checkout for wallet recharge
      ...
      // Send payment details to server for verification and wallet credit
  ...
  return (
    <div>
      ...
      {showRecharge && (
        <Card className="mb-6">
          <CardHeader><CardTitle className="text-base">Add Money to Wallet</CardTitle></CardHeader>
          <CardContent className="space-y-4">
            ...
            <Button variant="ghost" size="sm" onClick={() => setShowRecharge(false)}>Cancel</Button>
          </CardContent>
        </Card>
      )}
...
export default AgrizinMoneySection;
